...
class LapTimerGUI:
    ...
    def process_frame(self, frame):
        ...
        # OCR processing (from original code)
        config = '--psm 7 -c tessedit_char_whitelist=0123456789.:'
        time_text = pytesseract.image_to_string(thresh, config=config).strip()
        
        # Flash detection (from original code)
        mask = thresh.astype(bool)
        masked_pixels = roi_resized[mask]
        avg_color = np.mean(masked_pixels, axis=0) if masked_pixels.size else [0, 0, 0]
        r, g, b = avg_color
        
        # Alternative: detect any bright color flash
        avg_brightness = (r + g + b) / 3
        is_flash = (avg_brightness > 180 and (g > r + 20 or b > r + 20))
            ...
            if self.race_active:
                ...
                if not is_flash and stable_time_duration > self.FINAL_STABLE_SECONDS:
                    if len(self.lap_times) > 0 and time_text != self.lap_times[-1]:
                        ...
                        self.root.after(0, lambda: self.status_label.config(text="Race finished!"))
